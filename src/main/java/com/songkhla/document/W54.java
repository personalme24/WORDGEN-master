...
public class W54 {
    public static void w54(String cc) {
...
//              
		...
		bookmarkvalue.put("C2",Checknull(cs));
                ...
                bookmarkvalue.put("S6", Checknull(StationProvince));
                    ...
                    bookmarkvalue.put("PF7", ""); 
                    bookmarkvalue.put("PF13", "");
                    bookmarkvalue.put("PF14", "");
                    bookmarkvalue.put("PF15", "");
                    bookmarkvalue.put("PF16", "");
                    bookmarkvalue.put("PF17", "");
                    bookmarkvalue.put("PF22", "");
                    bookmarkvalue.put("PF23", "");
                    bookmarkvalue.put("PF24", "");
                    bookmarkvalue.put("PF25", "");
                    bookmarkvalue.put("PF26", "");
                    bookmarkvalue.put("PF28", "");
                    bookmarkvalue.put("PF79", "");
                    bookmarkvalue.put("PF104","");
                    bookmarkvalue.put("PF105","");
...
//			table1.add(repl2);
		...
		try {
			...
			WordprocessingMLPackage wordMLPackage = WordprocessingMLPackage
					.load(new java.io.File("./TEMPLATE/w54.docx"));
                        
			processVariable(bookmarkvalue,wordMLPackage);
			processTABLE(bookmarkvalue,wordMLPackage);
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/แบบแจ้งรูปพรรณคนหาย "+cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
...
//              
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/แบบแจ้งรูปพรรณคนหาย.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
	...
	private static void replaceTable(JSONArray placeholders, JSONArray data,
		...
		if (rows.size() == 2) {
         ...
         try {
             ...
             Logger.getLogger(W54.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
}
