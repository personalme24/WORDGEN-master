...
public class W56 {
    public static void w56(String cc) {
...
//               
            ...
            String sqlDataPolice="SELECT * FROM Police";
                  ...
                  while (rs1.next()) {                    
                        ...
                        String sql="select crimecase.*,ChargeCase.*,ActionsCaseData.*,InvestInformation.*\n" +
                              "from crimecase \n" +
...
//			table1.add(repl2);
		...
		try {
			...
			WordprocessingMLPackage wordMLPackage = WordprocessingMLPackage
					.load(new java.io.File("./TEMPLATE/w56.docx"));
                        
			processVariable(bookmarkvalue,wordMLPackage);
			processTABLE(bookmarkvalue,wordMLPackage);
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/แบบแจ้งผลการได้ตัวคนหายคืน "+cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
...
//              
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/แบบแจ้งผลการได้ตัวคนหายคืน.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
	...
	private static void replaceTable(JSONArray placeholders, JSONArray data,
		...
		if (rows.size() == 2) {
         ...
         try {
             ...
             Logger.getLogger(W56.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
}
