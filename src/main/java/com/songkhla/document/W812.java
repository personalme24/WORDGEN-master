...
public class W812 {
    public static void w812(String cc) {
...
//			table1.add(repl2);
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/บันทึกคำให้การพยาน "+s.getString("FullNamePerson")+""+ cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
            }
...
//                 bookmarkvalue.put("S27","-");
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/บันทึกคำให้การของพยาน.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
	...
	private static void replaceTable(JSONArray placeholders, JSONArray data,
        ...
        private static String ToDate(String strDate){
         ...
         try {
             ...
             Logger.getLogger(W812.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
}
