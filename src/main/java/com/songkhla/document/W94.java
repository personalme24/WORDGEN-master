...
public class W94 {
    public static void w94(String cc) {
...
//              
		...
		bookmarkvalue.put("C2",Checknull(cs));
                ...
                bookmarkvalue.put("S29", Checknull(THNumBook));
                   ...
                   /*
                                bookmarkvalue.put("P01", Checknull(IdCardPolice));
...
//			table1.add(repl2);
		...
		try {
			...
			WordprocessingMLPackage wordMLPackage = WordprocessingMLPackage
					.load(new java.io.File("./TEMPLATE/w94.docx"));
                        
			processVariable(bookmarkvalue,wordMLPackage);
			processTABLE(bookmarkvalue,wordMLPackage);
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/แบบตรวจรับของกลางยาเสพติดรายใหญ่ "+cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
...
//              
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/แบบตรวจรับของกลางยาเสพติดรายใหญ่.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
	...
	private static void replaceTable(JSONArray placeholders, JSONArray data,
		...
		if (rows.size() == 2) {
         ...
         try {
             ...
             Logger.getLogger(W94.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
}
