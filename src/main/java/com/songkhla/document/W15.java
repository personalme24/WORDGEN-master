...
public class W15 {
    public static void w15(String cc) {
...
//                   String sql="SELECT * from CrimeCase Where crimecaseno = '"+cc+"'";
                   String sql="select crimecase.*,ChargeCase.*,Asset.*,P1.*,P2.*,InvestInformation.*\n" +
                            ...
                            +   "FROM Person where Person.TypePerson='ผู้กล่าวหา'\n" +
                                ...
                                "where crimecase.CaseId='"+cc+"' and Asset.StatusAsset='เพลิงไหม้'\n" +
...
*/
		...
		try {
			...
			WordprocessingMLPackage wordMLPackage = WordprocessingMLPackage
					.load(new java.io.File("./TEMPLATE/w15.docx"));
			processVariable(bookmarkvalue,wordMLPackage);
			processTABLE(bookmarkvalue,wordMLPackage);
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/บัญชีทรัพย์ถูกเพลิงไหม้" +cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
            }
...
//              
                 ...
                 bookmarkvalue.put("PA7","");
                  ...
                  bookmarkvalue.put("AS10","");
                    ...
                    bookmarkvalue.put("AS1","");
                       ...
                       bookmarkvalue.put("PO12", "");
                       bookmarkvalue.put("PO13", "");
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/บัญชีทรัพย์ถูกเพลิงไหม้.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
            }
	...
	private static void replaceTable(JSONArray placeholders, JSONArray data,
		...
		if (rows.size() == 2) {
         ...
         try {
             ...
             Logger.getLogger(W15.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
    }
