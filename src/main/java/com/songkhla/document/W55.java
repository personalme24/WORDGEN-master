...
public class W55 {
    public static void w55(String cc) {
...
//			table1.add(repl2);
		...
		try {
			...
			WordprocessingMLPackage wordMLPackage = WordprocessingMLPackage
					.load(new java.io.File("./TEMPLATE/w55.docx"));
                        
			processVariable(bookmarkvalue,wordMLPackage);
			processTABLE(bookmarkvalue,wordMLPackage);
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/แบบการสืบสวนติดตามคนหาย "+cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
...
//              
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/แบบการสืบสวนติดตามคนหาย.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
	...
	private static void replaceTable(JSONArray placeholders, JSONArray data,
		...
		if (rows.size() == 2) {
         ...
         try {
             ...
             Logger.getLogger(W55.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
}
