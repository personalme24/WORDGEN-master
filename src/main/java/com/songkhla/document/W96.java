...
public class W96 {
    public static void w96(String cc) {
...
//               
            ...
            String sqlDataPolice="SELECT * FROM Police";
                  ...
                  while (rs1.next()) {      
                         IdCardPolice =rs1.getString("IdCardPolice");
      ...
      Statement st2 = conn.createStatement();
            ...
            ResultSet s=st.executeQuery(sql); 
               ...
               Time=sdfstart.format(calstart.getTime());
                ...
                bookmarkvalue.put("C1",Checknull(Time));
...
//			table1.add(repl2);
		...
		try {
			...
			WordprocessingMLPackage wordMLPackage = WordprocessingMLPackage
					.load(new java.io.File("./TEMPLATE/w96.docx"));
                        
			processVariable(bookmarkvalue,wordMLPackage);
			processTABLE(bookmarkvalue,wordMLPackage);
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/บันทึกการแจ้งสิทธิของผู้ถูกจับหรือผู้ต้องหา "+s.getString("FullNamePerson")+""+cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
            }
			...
			processVariable(bookmarkvalue,wordMLPackage);
			processTABLE(bookmarkvalue,wordMLPackage);
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/บันทึกการแจ้งสิทธิของผู้ถูกจับหรือผู้ต้องหา "+cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
                    }
...
//              
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/บันทึกการแจ้งสิทธิของผู้ถูกจับหรือผู้ต้องหา.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
	...
	private static void replaceTable(JSONArray placeholders, JSONArray data,
		...
		if (rows.size() == 2) {
         ...
         try {
             ...
             Logger.getLogger(W96.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
}
