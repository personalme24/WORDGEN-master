...
public class W18 {
     public static void w18(String cc) {
...
//			row1.put("REMARK", "period1");
		...
		try {
			...
			wordMLPackage.save(new java.io.File("./สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/บันทึกการตรวจสถานที่เกิดเหตุคดีจราจร"  +cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
            }
...
//              
    ...
    private static String ToDate(String strDate){
         ...
         try {
             ...
             Logger.getLogger(W18.class.getName()).log(Level.SEVERE, null, ex);
         }
               return ResultDate;
}
